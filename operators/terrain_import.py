...
from ..utils.logging import logger, sanitize_url
...
class GEODB_OT_ImportTerrain(GeoDBAsyncOperator):
    ...
    def invoke(self, context, event):
        ...
        logger.debug("Fetching available textures for project")
        ...
        if success:
            ...
            logger.debug("Cached %d textures for dialog", len(_available_textures_cache))
        else:
            # Clear cache - no textures available
            _available_textures_cache = []
            logger.debug("No textures available: %s", textures_data.get('error', 'Unknown error'))
    ...
    def download_data(self):
        ...
        try:
            ...
            logger.debug("selected_texture value = '%s'", self.selected_texture)
            logger.debug("available_textures_cache has %d items", len(_available_textures_cache))
            ...
            if self.selected_texture and self.selected_texture not in ('none', 'unavailable'):
                for tex in _available_textures_cache:
                    if str(tex.get('id', '')) == self.selected_texture:
                        ...
                        logger.debug("Found matching texture: %s", texture_name)
                        break

            if not texture_url and self.selected_texture not in ('none', 'unavailable'):
                logger.warning("Selected texture ID %s not found in cache", self.selected_texture)
            ...
            if texture_url:
                try:
                    ...
                    logger.warning("Failed to download texture: %s", e)
            ...
            logger.debug("Terrain mesh ready: %d vertices", len(mesh_data['positions']) // 3)
            logger.debug("Available textures: %d", len(_available_textures_cache))
            if texture_name:
                logger.debug("Selected texture: %s (%s)", texture_name, texture_type)
            ...
            if texture_path:
                try:
                    if os.path.exists(texture_path):
                        ...
                        if texture_path in _temp_files:
                            _temp_files.remove(texture_path)
    ...
    def _download_texture(self, url: str) -> str:
        ...
        logger.debug("Downloading texture from: %s", sanitize_url(url))
        logger.debug("Using authentication: %s", bool(headers))
    ...
    def finish_in_main_thread(self, context):
        ...
        try:
            ...
            if available_textures:
                ...
                logger.debug("Stored %d available textures on mesh object", len(available_textures))
            ...
            logger.debug("Texture path present: %s", texture_path is not None)
            if texture_path:
                file_exists = os.path.exists(texture_path)
                file_size = os.path.getsize(texture_path) if file_exists else 0
                logger.debug("Texture file exists: %s, size: %d bytes", file_exists, file_size)
            else:
                logger.debug("No texture path provided")

            if texture_path and os.path.exists(texture_path):
                file_size = os.path.getsize(texture_path)
                if file_size < 100:
                    logger.error("Texture file is too small (%d bytes), likely corrupt or empty", file_size)
                else:
                    try:
                        self._apply_terrain_texture(obj, texture_path)
                        logger.debug("Material applied successfully")
                        logger.debug("Material slots on object: %d", len(obj.data.materials))
                    except Exception as e:
                        logger.error("Failed to apply texture: %s", e)
                        logger.debug("Error details", exc_info=True)
                ...
                logger.debug("Texture not applied - path missing or file doesn't exist")
    ...
    def _generate_terrain_uvs(self, mesh: bpy.types.Mesh, vertices: list, bounds: dict):
        ...
        logger.debug("Generating UVs for terrain: X(%.2f to %.2f), Y(%.2f to %.2f)",
        ...
        logger.debug("Generated UV coordinates for %d faces", len(mesh.polygons))
            ...
            if area.type == 'VIEW_3D':
                for space in area.spaces:
                    if space.type == 'VIEW_3D':
                        ...
                        logger.debug("Set viewport shading to Solid with Texture display")
    ...
    def _apply_terrain_texture(self, obj: bpy.types.Object, texture_path: str):
        ...
        # Clean up temp file after packing into .blend
        try:
            if texture_path and os.path.exists(texture_path):
                os.remove(texture_path)
                if texture_path in _temp_files:
        ...
        logger.debug("Applied texture material")
...
class GEODB_OT_SwitchTerrainTexture(GeoDBAsyncOperator):
    ...
    def invoke(self, context, event):
        ...
        if available_textures_json:
            try:
                ...
                logger.debug("Loaded %d textures from mesh object", len(_switch_textures_cache))
            except json.JSONDecodeError:
                _switch_textures_cache = []
                logger.warning("Failed to parse available_textures JSON from mesh object")
        else:
            _switch_textures_cache = []
            logger.debug("No available_textures stored on mesh object")
    ...
    def _apply_plain_material(self, obj: bpy.types.Object):
        ...
        logger.debug("Applied plain material to terrain")
    ...
    def _ensure_terrain_uvs(self, obj: bpy.types.Object):
        ...
        if mesh.uv_layers:
            logger.debug("Terrain already has UV layer: %s", mesh.uv_layers[0].name)
            return

        logger.debug("Terrain missing UV layer - generating planar projection UVs...")
