...
class GEODB_OT_ImportDrillPads(GeoDBAsyncOperator):
    ...
    def finish_in_main_thread(self, context):
        ...
        for pad in pads:
            ...
            if vertices_2d:
                print(f"  First vertex: {vertices_2d[0]}, Last vertex: {vertices_2d[-1]}")
    ...
    def finish_in_main_thread(self, context):
        ...
        for server_hole in results['create_local']:
            ...
            if not collar:
                ...
                print(f"Warning: Could not find pad '{pad_name}' in scene for hole {hole_name}. "
                      f"Import the pad first to pull this hole.")
                continue
...
def on_mode_change_handler(scene, depsgraph):
    ...
    for obj in bpy.data.objects:
        ...
        if last_mode == 'EDIT' and current_mode == 'OBJECT':
            ...
            print(f"Detected Edit Mode exit for {obj.name}, updating geometry...")
            ...
            def delayed_update():
                try:
                    ...
                    print(f"Error updating hole geometry: {e}")
